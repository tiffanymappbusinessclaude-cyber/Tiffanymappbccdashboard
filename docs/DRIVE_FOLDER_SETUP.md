...
> The Email Archiver recipe uses `internal_handler = 'dispatch_email_archiver'`. As of 2026-07-02, that Postgres function is **not defined in any master migration** (only `gl_entry_writer`, `monthly_close_monitor`, and `producer_underperformance_watcher` ship in migration 012). Six other internal handlers used by the seeded recipes are also undefined at master: `dispatch_document_processor`, `bank_gl_writer`, `cc_gl_writer`, `payroll_gl_writer`, `monthly_close_generator`, `instagram_manual_reminder`. Firing a recipe whose handler is not defined will error with `function does not exist`. Before running the manual trigger test, either (a) target a recipe with a known-good handler (`GL Entry Writer` uses `gl_entry_writer`), or (b) ask Rebecca whether the missing handlers should be pulled from a reference client install (Keith Thompson) and back-ported to master as a new migration. Tracked as audit finding B8.
