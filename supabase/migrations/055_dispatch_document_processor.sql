-- =========================================================================
-- Migration: 015_dispatch_document_processor
-- Supabase version: 20260616211049
-- Captured from production DB: 2026-06-17
-- Note: superseded by 015b (extended timeout) and 015c (sync collect).
-- =========================================================================

-- Migration 015: dispatch_document_processor (skeleton)
-- Fetches recent emails from Composio Gmail, classifies them by sender/subject,
-- and queues them in the documents table with processing_status='classified'.
-- Typed parsers (sf_daily_comp, payroll, bank, cc, deduction, producer) are
-- DEFERRED — they will populate source tables in a follow-up migration.
-- =====================================================================
...
BEGIN
  ...
  IF v_status_code IS NULL THEN
    RETURN jsonb_build_object(
      'records_processed', 0,
      'output_summary',    'Timed out waiting for Composio (request_id ' || v_request_id::text || ')'
    );
  END IF;
