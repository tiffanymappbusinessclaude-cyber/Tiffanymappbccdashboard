-- v10.055 — Rewire producer_underperformance_watcher to read from goals (not empty producer_production)
--
-- Original recipe was blind: producer_production has 0 rows; production data lives in public.goals.
-- New algorithm: for each active producer, evaluate their 2026 production goals against tenure-adjusted
-- year-pace. Flag any goal whose current_value < 70% of expected pace. One alert per producer per day.
--
-- Tenure proration uses staff.start_date (backfilled 2026-07-11 from persistent_memory rows Tiffany
-- confirmed 2026-07-09):
--   pace_factor = (today - GREATEST(start_date, jan_1_curr_year) + 1) / (dec_31_curr_year - that_start + 1)
--
-- Idempotency: NOT EXISTS check on module_reference + is_resolved=false + created_at::date=today
-- (correct semantic here: "refresh the flag daily", not "one flag per issue ever").
...
BEGIN
  ...
  LOOP
    ...
    IF jsonb_array_length(v_flagged_goals) > 0 THEN
      ...
      SELECT
        ...
        'warning',
        v_producer.name || ': ' || jsonb_array_length(v_flagged_goals) || ' production goal(s) below 70% pace',
        'Through day ' || v_day_of_month || ' at ' || ROUND(v_pace_factor*100, 0) ||
  ...
  RETURN jsonb_build_object(
    ...
    'output_summary', v_alert_count || ' producers flagged underperforming (goals-based, tenure-adjusted)'
  );
END;
$function$;

COMMENT ON FUNCTION public.producer_underperformance_watcher(uuid, uuid) IS
  'Producer performance watcher: compares goals.current_value against tenure-adjusted pace on 2026 production goals per producer. Rewired 2026-07-11 from empty producer_production table to goals table. Uses staff.start_date for tenure. Threshold: <70% pace flags. One alert per producer per day.';
