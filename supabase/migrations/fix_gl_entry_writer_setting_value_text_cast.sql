-- Hotfix: settings.setting_value is TEXT (not jsonb), and stored as `"2026-07-01"`
-- with literal double-quotes. Use trim() before casting to date.
...
BEGIN
  ...
  SELECT trim(both '"' from setting_value)::date INTO v_cutover_date
  ...
  SELECT trim(both '"' from setting_value) INTO v_cash_code
  FROM public.settings
