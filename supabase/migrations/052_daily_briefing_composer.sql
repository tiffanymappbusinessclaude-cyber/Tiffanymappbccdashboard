-- =========================================================================
-- MIGRATION 052 — Daily Briefing Composer
-- =========================================================================
-- Ships the composer that recipe seed 02 (Daily Briefing Email) has always
-- required. Prior to this migration, seed 02 shipped with is_active=true and
-- composio_action='GMAIL_SEND_EMAIL' but no runner code or SQL function to
-- compose the HTML body from live data — so the recipe failed on first run
-- and setup leads had to close the gap during install. This migration closes
-- it in master template so install #31 onward ships with a working briefing.
--
-- WHAT THIS MIGRATION ADDS:
--   1. public.briefings — one row per (agency_id, briefing_date) with the
--      rendered HTML body, subject, recipient, and pg_net request id.
--      Idempotent on same-day re-runs.
--   2. public.daily_briefing_composer(p_agency_id, p_recipe_id) RETURNS jsonb
--      — the INTERNAL handler invoked by run_internal_recipe (from migration
--      012) when Daily Briefing Email fires. Reads the recipe's input_config
--      for tz + recipient_email, aggregates live business data (revenue YTD /
--      MTD, AIPP pace, open tasks, active alerts, upcoming compliance, active
--      staff), composes an HTML body using the agency's owner_name and name
--      from public.agency, upserts the briefings row, and fires the send via
--      pg_net.http_post against Composio GMAIL_SEND_EMAIL. Returns the jsonb
--      shape enforced by run_internal_recipe: {records_processed, output_summary}.
--
-- REQUIREMENTS (verified in master as of migration 051):
--   - public.agency (created in migration 001, owner_name is NOT NULL)
--   - public.get_setting() (created in migration 011)
--   - public.automation_recipes.internal_handler column (added in migration 012)
--   - pg_net extension (enabled in migration 016)
--   - public.comp_recap, public.aipp_tracking, public.tasks, public.alerts,
--     public.compliance_calendar, public.staff (all created in migration 001)
--
-- COMPANION SEED CHANGE (in this same commit):
--   supabase/recipe_seeds/02_daily_briefing_email.sql — composio_action
--   changed from 'GMAIL_SEND_EMAIL' to 'INTERNAL' so run_internal_recipe
--   routes to this composer via the internal_handler column.
--
-- ATTRIBUTION: pattern adapted from a working client install (migration 013b,
-- deployed 2026-06-16). Master template is agency-neutral — all client-specific
-- hardcoding removed; owner name and agency name come from public.agency,
-- resolved at composer runtime.
-- =========================================================================
...
CREATE INDEX IF NOT EXISTS briefings_agency_date_idx
  ON public.briefings (agency_id, briefing_date DESC);

COMMENT ON TABLE public.briefings IS
  'Rendered daily briefing emails, one row per (agency_id, briefing_date). Idempotent — same-day re-runs UPSERT.';


-- =========================================================================
-- daily_briefing_composer(agency_id, recipe_id) RETURNS jsonb
-- =========================================================================
-- Contract enforced by run_internal_recipe (migration 012):
--   Input:  p_agency_id UUID, p_recipe_id UUID
--   Output: jsonb { records_processed: int, output_summary: text }
-- =========================================================================
...
BEGIN
  ...
  -- Load agency identity (owner_name is NOT NULL per migration 001)
  ...
  IF v_owner_name IS NULL THEN
    RAISE EXCEPTION 'daily_briefing_composer: agency % not found or owner_name missing', p_agency_id;
  END IF;
  ...
  -- Gather metrics (all source tables created in migration 001)
  ...
  v_subject := 'Daily Briefing — ' || COALESCE(v_agency_name, 'Agency')
               || ' — ' || to_char(v_today, 'Mon DD, YYYY');
...
<html><body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif; color: #1a1a1a; max-width: 640px; margin: 0 auto; padding: 20px;">
  ...
  <p style="margin-top: 36px; color: #999; font-size: 12px; border-top: 1px solid #e5e5e5; padding-top: 14px;">
    %s&#39;s Claude &middot; Business Command Center &middot; briefing for %s
  </p>
</body></html>$html$,
    v_owner_name,
  ...
  -- Record sent_at + pg_net request_id (response correlation via net._http_response)
  ...
  RETURN jsonb_build_object(
    'records_processed', 1,
    'output_summary',    'Sent: "' || v_subject || '" to ' || v_recipient
                         || ' (briefing_id=' || v_briefing_id::text
                         || ', pg_net_request_id=' || v_request_id::text || ')'
  );
END;
$func$;

GRANT EXECUTE ON FUNCTION public.daily_briefing_composer(UUID, UUID)
  TO postgres, service_role;

COMMENT ON FUNCTION public.daily_briefing_composer(UUID, UUID) IS
  'INTERNAL handler for the Daily Briefing Email recipe. Called by run_internal_recipe(recipe_id) from migration 012. Composes HTML from live business data, upserts briefings row, sends via pg_net to Composio Gmail.';
