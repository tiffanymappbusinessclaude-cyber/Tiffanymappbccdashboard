-- bcc-deepdive/P7 — 2026-07-24T04:25Z
-- Finding F12: bank_gl_writer + cc_gl_writer report "success" with
-- silent unmapped-skip counts every day. F1's 281 stuck txns went
-- 3+ days undetected. Fix: append dedup-guarded alert insert before
-- RETURN when unmapped-skip > 0. One alert per module_reference per
-- unresolved-window. Follows the producer_underperformance_watcher
-- pattern from bcc_debug_audit_2026_07_24_producer_watcher_dedup.
-- Rollback: restore prior bodies from context of session 09e3c11d.

-- ==========================================================
-- bank_gl_writer — add alert when unmapped-skip > 0
-- ==========================================================
...
BEGIN
  ...
  -- === P7 alert insert (dedup on module_reference + is_resolved=false) ===
  IF v_skipped_unmapped > 0 THEN
    IF EXISTS (
      SELECT 1 FROM public.alerts
      WHERE agency_id = p_agency_id
        AND alert_type = 'gl_writer_stuck_unmapped'
        AND module_reference = 'bank_gl_writer'
        AND is_resolved = false
    ) THEN
      UPDATE public.alerts
        SET title   = 'Bank GL Writer: ' || v_skipped_unmapped || ' unpostable txns (bank_account CoA unmapped)',
            message = 'bank_gl_writer cannot post ' || v_skipped_unmapped || ' new bank_transactions because bank_accounts.chart_of_accounts_id is NULL. Fix: assign chart_of_accounts_id on affected bank_accounts, then rerun. Last refreshed by writer: ' || v_now::date
      WHERE agency_id = p_agency_id
        AND alert_type = 'gl_writer_stuck_unmapped'
        AND module_reference = 'bank_gl_writer'
        AND is_resolved = false;
    ELSE
      INSERT INTO public.alerts (agency_id, alert_type, severity, title, message, module_reference, is_resolved, created_at)
      VALUES (
        p_agency_id, 'gl_writer_stuck_unmapped', 'high',
        'Bank GL Writer: ' || v_skipped_unmapped || ' unpostable txns (bank_account CoA unmapped)',
        'bank_gl_writer cannot post ' || v_skipped_unmapped || ' new bank_transactions because bank_accounts.chart_of_accounts_id is NULL. Fix: assign chart_of_accounts_id on affected bank_accounts, then rerun.',
        'bank_gl_writer', false, v_now
      );
    END IF;
  ELSE
    -- Auto-resolve any prior stuck alert if the condition has cleared
    UPDATE public.alerts
  ...
  RETURN jsonb_build_object(
    ...
    'output_summary',
      ...
      || CASE WHEN v_skipped_unmapped > 0 THEN '; ' || v_skipped_unmapped || ' skipped (bank_account.chart_of_accounts_id unmapped) [ALERT RAISED]' ELSE '' END
  );
END;
$function$;

-- ==========================================================
-- cc_gl_writer — add alert when unmapped-skip > 0
-- ==========================================================
...
BEGIN
  ...
  IF v_skipped > 0 THEN
    IF EXISTS (
      ...
      WHERE agency_id = p_agency_id
        AND alert_type = 'gl_writer_stuck_unmapped'
        AND module_reference = 'cc_gl_writer'
        AND is_resolved = false
    ) THEN
      UPDATE public.alerts
        SET title   = 'Credit Card GL Writer: ' || v_skipped || ' unpostable txns (credit_account CoA unmapped)',
            message = 'cc_gl_writer cannot post ' || v_skipped || ' new credit_transactions because credit_accounts.chart_of_accounts_id is NULL. Fix: assign chart_of_accounts_id on affected credit_accounts, then rerun. Last refreshed by writer: ' || v_now::date
      WHERE agency_id = p_agency_id
        AND alert_type = 'gl_writer_stuck_unmapped'
        AND module_reference = 'cc_gl_writer'
        AND is_resolved = false;
    ELSE
      ...
      VALUES (
        p_agency_id, 'gl_writer_stuck_unmapped', 'high',
        'Credit Card GL Writer: ' || v_skipped || ' unpostable txns (credit_account CoA unmapped)',
        'cc_gl_writer cannot post ' || v_skipped || ' new credit_transactions because credit_accounts.chart_of_accounts_id is NULL. Fix: assign chart_of_accounts_id on affected credit_accounts, then rerun.',
        'cc_gl_writer', false, v_now
      );
    END IF;
  ELSE
    UPDATE public.alerts
  ...
  RETURN jsonb_build_object(
    ...
    'output_summary',
      ...
      || CASE WHEN v_skipped > 0 THEN '; ' || v_skipped || ' skipped (credit_account.chart_of_accounts_id unmapped) [ALERT RAISED]' ELSE '' END
  );
END;
$function$;
