...
function PromptCard({ prompt, sectionTitle, subsectionTitle, highlight }) {
  ...
  return (
    <div
      className="playbook-prompt-card"
      style={{
        padding: '18px 20px',
        marginBottom: '12px',
        backgroundColor: 'var(--if-surface)',
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', marginBottom: '10px', flexWrap: 'wrap' }}>
        <div style={{ fontWeight: 700, fontSize: '15px', color: 'var(--if-navy)', flex: 1, minWidth: '200px', lineHeight: 1.35 }}>
        ...
        <span
          style={{
            ...
            color: 'white',
            background: 'linear-gradient(135deg, var(--if-coral) 0%, #E85D75 100%)',
            padding: '4px 10px',
            borderRadius: '999px',
            whiteSpace: 'nowrap',
            boxShadow: '0 1px 3px rgba(232,93,117,0.25)',
          }}
        >
          {prompt.tag}
        </span>
      </div>
      <div style={{ fontSize: '14px', lineHeight: 1.65, color: 'var(--if-ink)', marginBottom: prompt.tip ? '12px' : '14px' }}>
      ...
      {prompt.tip && (
        <div
          style={{
            ...
            padding: '10px 12px',
            borderRadius: '8px',
            backgroundColor: 'var(--if-cream)',
            border: '1px solid var(--if-coral-lt)',
            display: 'flex',
            alignItems: 'flex-start',
            gap: '8px',
            lineHeight: 1.55,
        ...
        >
          <Lightbulb size={13} style={{ marginTop: 2, flexShrink: 0, color: 'var(--if-coral)' }} />
          <div><em>Tip:</em> {prompt.tip}</div>
        </div>
      )}
      <div>
        <AskClaudeButton
...
export default function PlaybookGuide() {
  ...
  function jumpTo(id) {
    ...
    const el = document.getElementById('playbook-section-' + id);
    if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }

  return (
    <div style={{ padding: '0 0 48px', maxWidth: '960px', margin: '0 auto' }}>
      {/* Hero header */}
      <div
        style={{
          position: 'relative',
          background: 'linear-gradient(135deg, #1B2B4B 0%, #2D3E63 55%, #F5946A 130%)',
          color: 'white',
      ...
      >
        <div style={{ position: 'absolute', top: -30, right: -30, opacity: 0.08, transform: 'rotate(-12deg)', pointerEvents: 'none' }}>
          <BookOpen size={200} />
        </div>
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
            <Sparkles size={20} />
            <span style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', opacity: 0.92 }}>
              Playbook &amp; Guide
            </span>
          </div>
          <h1 style={{ margin: 0, fontSize: '28px', fontWeight: 800, lineHeight: 1.2 }}>
            How to work with your Claude, in plain English.
          </h1>
          <p style={{ fontSize: '14px', margin: '10px 0 20px', opacity: 0.92, lineHeight: 1.6, maxWidth: '640px' }}>
            {PLAYBOOK_DATA.length} sections · {totalPrompts} ready-to-use prompts. Every one has a
            one-click <em>Try in Claude</em> button. Search anything, jump anywhere.
          </p>
          ...
          <div style={{ position: 'relative', marginBottom: '14px' }}>
            <Search
              size={18}
              style={{ position: 'absolute', left: '16px', top: '50%', transform: 'translateY(-50%)', color: 'rgba(15,23,42,0.55)', pointerEvents: 'none' }}
            />
            <input
              type="text"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Search prompts, tags, tips, or section titles…"
              style={{
                width: '100%',
            ...
            {query && (
              ...
              >
                <X size={14} color="var(--if-navy)" />
              </button>
            )}
          </div>

          {/* Stats + expand toggle */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px', fontSize: '12px', opacity: 0.94, flexWrap: 'wrap' }}>
            {query.trim() ? (
              <div>
                <strong style={{ fontSize: '15px', marginRight: 4 }}>{visibleSectionCount}</strong>
                matching section{visibleSectionCount === 1 ? '' : 's'} for "{query}"
              </div>
            ) : (
              <>
                <div>
                  <strong style={{ fontSize: '15px', marginRight: 4 }}>{PLAYBOOK_DATA.length}</strong>
                  sections
                </div>
                <div>
                  <strong style={{ fontSize: '15px', marginRight: 4 }}>{totalPrompts}</strong>
                  prompts
                </div>
                <div style={{ marginLeft: 'auto' }}>
                  ...
                  >
                    {expandAll ? <ChevronsUp size={13} /> : <ChevronsDown size={13} />}
      ...
      {/* Quick-jump nav — hides during active search */}
      {!query.trim() && (
        <div
      ...
      <div>
        {PLAYBOOK_DATA.map((section, idx) => (
          <div id={'playbook-section-' + section.id} key={section.id} style={{ scrollMarginTop: '20px' }}>
            <Section
              section={section}
              query={query.trim()}
      ...
      <div
        style={{
          ...
          padding: '20px 22px',
          borderRadius: '14px',
          background: 'linear-gradient(135deg, var(--if-cream) 0%, #FFF7ED 100%)',
          border: '1px solid var(--if-coral-lt)',
          fontSize: '13px',
          color: 'var(--if-ink)',
          lineHeight: 1.7,
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
          <Info size={14} color="var(--if-coral)" />
      ...
      <style>{`
        ...
        .playbook-prompt-card:hover {
          border-color: var(--if-coral-lt) !important;
